...
scales = [1, -0.05, 0.85, 0.8, -0.1, 0.1 ];
...
for i = 1 : nImages
    ...
    for j = 1 : scales_size
        ...
        for n=1:numel(inds)        
            ...
            %filter out under 80% confidence
            if confidence < -0.5
                continue
            end
        ...
        for k = 1 : detection_count
            if (valid_bound_boxes(k) == 1)
                ...
                if (left < right && top < bottom)
                    ...
                    center_coord = [(bound_box(1) + bound_box(3))/2, (bound_box(2) + bound_box(4))/2];
                    if( center_coord(1) > other_bound_box(1) && center_coord(1) < other_bound_box(3) && ...
                        center_coord(2) > other_bound_box(2) && center_coord(2) < other_bound_box(4))
                        is_valid = false;
                    end
                end
    ...
    hard_negative_confidences = confidences;
end

save('hard_negatives.mat','bound_boxes','hard_negative_confidences','image_names');
